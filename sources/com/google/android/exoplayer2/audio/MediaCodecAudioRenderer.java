...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    public int supportsFormat(MediaCodecSelector mediaCodecSelector, Format format) {
        ...
        return i | 8 | (Util.SDK_INT < 21 || ((format.sampleRate == -1 || decoderInfo.isAudioSampleRateSupportedV21(format.sampleRate)) && (format.channelCount == -1 || decoderInfo.isAudioChannelCountSupportedV21(format.channelCount))) ? 4 : 3);
    }
    ...
    public long getPositionUs() {
        ...
        if (currentPositionUs2 != Long.MIN_VALUE) {
            if (this.allowPositionDiscontinuity) {
                j = currentPositionUs2;
            } else {
                j = Math.max(this.currentPositionUs, currentPositionUs2);
            }
            this.currentPositionUs = j;
